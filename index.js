...
/**
 * @typedef {Object} KanaKanjiString
...
async function transformFuriganaAsync(normalizedSingleWord) {
  ...
  // 把括号和其中的内容变成空字符串, Sample: こし(少し) => こし
  const kana = normalizedSingleWord.replace(/\([^\)]+\)/g, '').trim()
  // 匹配括号中的内容 Sample: こし(少し) => 少し
...
async function __testTransformFuriganaAsync() {
  for (const [input, expect] of [
    ['うち', 'うち'],
    ['こし(少し)', '少[こ] し'],
    ['お／ご～もうしあげる（お／ご～申し上げる）', 'お/ご～ 申し上[もうしあ] げる'],
    ...
    ['よろしくおつたえください(～お伝えください)', 'よろしくお 伝[つた] えください'],
    ['さどトキほごセンター（佐渡～保護～）', '佐渡[さど] トキ 保護[ほご] センター'],
  ]) {
...
async function _transformPlainFuriganaWordAsync(normalizedWord) {
  if (normalizedWord.match(/\([^\)\/]+\//)) {
    ...
    const transformedSubWords = await Promise.all(subWords.map(transformFuriganaAsync))
...
async function transformPlainFuriganaWordAsync(normalizedWord) {
  ...
  assert.equal(extractKanaFromBookFormat(normalizedWord), extractKanaFromAnkiFormat(result))
  return result
}

/**
 * 把注音假名单词转换为Anki格式的测试用例
 * @async
 * @returns {Promise<void>}
 */
...
function extractKanaFromAnkiFormat(w) {
  return w
    ...
    .map((seg) => {
      if (seg.indexOf('[') > -1) {
        return [...seg.matchAll(/\[([^\]]+)\]/g)].map((v) => v[1].trim()).join('')
      } else {
...
async function __testTransformXmlFuriganaWordAsync() {
  for (const [input, expect] of [
    ["<xml><tag name='common' superscript='だい'>第</tag><tag name='common'>～  </tag></xml>", '第[だい] ～'],
    [
...
async function processBook(bookName, bookId, cipherKey, basePath) {
  log.start('Process book %s in %s', chalk.green(bookName), chalk.green(basePath))
  ...
  for (const lessonPath of lessonPaths) {
    ...
    for (let wordIdx = 0; wordIdx < words.length; wordIdx++) {
      ...
      const audioName = [`book${pad(bookId)}`, `lesson${pad(lessonIdx)}`, `${pad(wordIdx)}.mp3`].join('-')
      ...
      if (wordItem.word.indexOf('～') > -1) {
        if (ankiWord.indexOf('～') === -1) {
          log.note('Infer success: %s → %s', wordItem.word, chalk.yellow(ankiWord))
        } else {
  ...
  fs.writeFileSync(
    ...
    notes
      .map((data) =>
        [data.id, data.jp, data.type, data.trans, data.lesson, `[sound:${data.audio}]`, data.tag].join('\t')
      )
      .join('\n')
  )
}
